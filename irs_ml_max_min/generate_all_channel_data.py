...
from max_min_bl.generate_received_pilots import generate_received_pilots_batch, channel_estimation_lmmse, channel_complex2real
from max_min_bl.generate_received_pilots import compute_stat_info,generate_pilots_bl_v2,generate_pilots_bl
import os
...
def main_channel_estimation(params_system, len_pilot, noise_power_db, Pt, location_user, Rician_factor, path):
    ...
    path = path[0:-4]
    file_path = path + '_' + str(len_pilot) + '_' + str(noise_power_db) + '.mat'
    sio.savemat(file_path,
                {'channel_bs_user': channel_bs_user_est,
                 'channel_bs_irs_user': channel_bs_irs_user_est})
    return file_path,err_bs_user,err_bs_irs_user
...
def main():
    ...
    len_pilots_set = np.array([5,25])*num_user

    err_bs_user_set,err_bs_irs_user_set,err_sum_set = [],[],[]
    for len_pilots in len_pilots_set:
        _, err_bs_user,err_bs_irs_user = main_channel_estimation(params_system, len_pilots, noise_power_db, Pt, location_user, Rician_factor, file_path)
