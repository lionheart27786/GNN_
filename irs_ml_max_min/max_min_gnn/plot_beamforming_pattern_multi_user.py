...
def load_nn_model(params_system,len_pilot, sigma2, model_path, input_flag):
    (num_antenna_bs, num_elements_irs, num_user) = params_system
    nn = MaxminModel(num_antenna_bs, num_elements_irs, num_user, len_pilot, input_flag, sigma2)
    nn.create_network()
...
#     phi_3_set = np.block([np.linspace(-np.pi/2, np.pi/2, num_points), np.array(phi_3_true)])
#     phi_3_set = np.sort(phi_3_set)
#
#     array_response = np.zeros((num_points+3, num_points+3))
#
...
def run_1():
    ...
    (num_antenna_bs,num_elements_irs,num_user) = params_system
    Rician_factor = 10
    location_user = np.array([[5,-12,-20],[12,0,-20],[5, 12,-20]])
    input_flag = 0
    len_pilot = 75
    noise_power_db = -100
    Pt_u = 15
    ...
    nn = load_nn_model(params_system,len_pilot, sigma2, model_path, input_flag)
    ...
    ### from users to bs
    aoa_irs_y, aoa_irs_z,aoa_irs_cos = [],[],[]
    for kk in range(num_user):
        d_k = np.linalg.norm(location_user[kk] - location_irs)
        aoa_irs_y_k = (location_user[kk][1] - location_irs[1]) / d_k
        aoa_irs_y.append(aoa_irs_y_k)
        aoa_irs_z_k = (location_user[kk][2] - location_irs[2]) / d_k
        aoa_irs_z.append(aoa_irs_z_k)
        aoa_irs_cos.append(np.linalg.norm(location_user[0][0:2] - location_irs[0:2]) / d_k)

    print('aoa_irs_y_k','aoa_irs_z_k',aoa_irs_y,aoa_irs_z)
    ...
    phi_3 = []
    for kk in range(num_user):
        phi_3_k = np.arcsin((location_user[kk][1]-location_irs[1])/np.linalg.norm(location_user[kk][0:2]-location_irs[0:2]))
        phi_3.append(phi_3_k)
    ...
    sio.savemat('array_response_irs'+str(location_user.reshape(9,))+ '.mat',{ 'array_response_irs2': array_response_irs2,
    ...
    for ii in range(num_user):
        aoa_aod = (aoa_bs, aod_irs_y, aod_irs_z, aoa_irs_y[ii], aoa_irs_z[ii])
        tmp1, tmp2 = plot_array_response_bs(w_nn[0][:,ii], params_system,aoa_aod, phi,irs_cos_z, irs_Nh=10,num_points=1000)
        array_response_bs.append(tmp1)
        set_phi_1.append(tmp2)
    ...
    #
    #
    sio.savemat('array_response_bs'+str(location_user.reshape(9,))+ '.mat',{'array_response_bs':array_response_bs,
                                      'set_phi_1':set_phi_1,
                                      'phi_true':phi
                                      })
